...
mod tests {
	...
	#[test]
	fn response_preserves_existing_output_text_annotations() {
		let response: Response = serde_json::from_value(serde_json::json!({
			"id": "resp_123",
			"status": "completed",
			"model": "gpt-5.6",
		...
		.expect("existing annotations should deserialize");
		...
		assert_eq!(
			serde_json::to_value(&response).unwrap()["output"][0]["content"][0]["annotations"][0],
			serde_json::json!({
				"type": "url_citation",
				"start_index": 4,
				"end_index": 10,
